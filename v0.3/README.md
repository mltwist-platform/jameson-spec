...
Compressed (string-encoded) RLE is not part of v0.3; producers must emit the uncompressed array form.
...
Compound conditions (`all_of` / `any_of`) are reserved for a future minor version; v0.3 conditions are a single clause by design.
...
| ------------ | ------ | ---------------------------------------------------- |
| `source`     | string | `"human"`, `"model"`, `"consensus"`, `"review"`, …   |
...
A JSON Schema for JAMESON v0.3 will be published alongside this specification. (JSON Schema cannot express rules 4, 7, 8, and 9 — reference validators must implement them in code.)
...
**End of JAMESON Annotation Format Specification Draft v0.3**
